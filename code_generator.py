...
class CodeGenerator:
        ...
        if len(node.children) > 0:
            ...
            if current:
                if isinstance(node, TypedNode):
                    ...
                    v_type = self.get_type(node.v_type)[0]
    ...
    def generate_array(self, node: ArrayNode, current: FunctionCode = None):
        pass

    def generate_function(self, node: TypedFuncDeclNode):
        params = [(*self.get_type(x.vars_list[0].v_type), x.vars_list[0].name) for x in node.params]
